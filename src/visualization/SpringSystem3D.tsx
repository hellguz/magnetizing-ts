...
import type { EvolutionaryFloorplanSolver } from "../core/solvers/EvolutionaryFloorplanSolver.js";

interface SpringSystem3DProps {
  solverRef: React.MutableRefObject<
    SpringSolver | EvolutionaryFloorplanSolver | null
  >;
...
// Coordinate system scale: 30 internal units approx equals 1 meter
// This aligns with the templates where ~300 area units = ~10m²
...
export const SpringSystem3D: React.FC<SpringSystem3DProps> = ({
  ...
  const adjacencyLabelRefsMap = useRef<Map<number, any>>(new Map());
  ...
  useFrame(() => {
    ...
    // Update adjacency lines and labels imperatively
    adjacencies.forEach((adj, index) => {
      const line = lineRefsMap.current.get(index);
      const label = adjacencyLabelRefsMap.current.get(index);
      ...
      if (roomA && roomB) {
        ...
        // 2. Update Label (Gap or Shared Wall)
        if (label) {
          ...
          // If the angle is > 90 or < -90 degrees, flip it 180 degrees
          ...
          // Calculate Gap in Raw Units
          const gapX = Math.max(
            0,
            Math.abs(centerA.x - centerB.x) - (roomA.width + roomB.width) / 2
          );
          const gapY = Math.max(
            0,
            Math.abs(centerA.y - centerB.y) - (roomA.height + roomB.height) / 2
          );
          const gap = Math.sqrt(gapX * gapX + gapY * gapY);

          // Convert to Meters
          const gapMeters = gap / PIXELS_PER_METER;

          if (gapMeters > 0.05) {
            // Show Gap
            label.text = `gap: ${gapMeters.toFixed(1)}m`;
            label.color = "#c62828"; // Dark Red
          } else {
            // Show Shared Wall
            // Calculate Overlap on Axes (Raw Units)
            ...
            // Shared wall length is the max overlap
            const sharedWall = Math.max(overlapX, overlapY);

            // Convert to Meters
            const sharedWallMeters = sharedWall / PIXELS_PER_METER;

            label.text = `share: ${sharedWallMeters.toFixed(1)}m`;

            // Target heuristic (using 1.5m as the target threshold)
            if (sharedWallMeters >= 1.5) {
              label.color = "#2e7d32"; // Dark Green (Good)
            } else {
              label.color = "#ef6c00"; // Dark Orange (Partial/Small share)
            }
          }
        }
      }
  ...
  return (
    <>
      ...
      {showBoundary && boundary && boundary.length > 0 && (
        <Line
          points={[
            ...boundary.map((p) => [p.x, p.y, 0]),
            [boundary[0].x, boundary[0].y, 0],
          ]}
          color="#2c2c2cff"
          lineWidth={3}
        />
      ...
      {/* Render adjacency lines and labels - will be updated imperatively via refs */}
      {showAdjacencies &&
        adjacencies.map((adj, index) => {
          ...
          return (
            <React.Fragment key={`adj-${index}`}>
              ...
              <Text
                ref={(text) => {
                  if (text) adjacencyLabelRefsMap.current.set(index, text);
                }}
                position={[
                  (centerA.x + centerB.x) / 2,
                  (centerA.y + centerB.y) / 2,
